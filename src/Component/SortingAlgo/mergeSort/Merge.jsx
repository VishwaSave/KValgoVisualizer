...
export default function Merge(props) {
  ...
  const generateRandomColor = () => {
    ...
    const maxRGB = 215;
    const randomColor = () =>
      Math.floor(Math.random() * (maxRGB - minRGB + 1)) + minRGB;
  ...
  const renderElements = () => {
    ...
    for (let i = 0; i < a.length; i++) {
      ...
      } while (elementColors.includes(uniqueColor));
  ...
  async function mergeSort(resp) {
    ...
    if (resp) {
      ...
      middle = Math.floor(resp.length / 2);
    ...
    return merge(await mergeSort(l), await mergeSort(r));
  }
  ...
  async function merge(l, r) {
    ...
    let lColor = [],
      rColor = [];
    l.map((val, ind) => {
      left.push(parseInt(val.innerText));
      lColor[ind] = val.style.backgroundColor;
    });
    r.map((val, ind) => {
      right.push(parseInt(val.innerText));
    ...
    while (leftIndex < left.length && rightIndex < right.length) {
      ...
      if (left[leftIndex] <= right[rightIndex]) {
        ...
        for (let k = aa.children.length - 1; k >= 0; k--) {
          if (aa.children[k].classList.contains("new")) {
            break;
          }
          count--;
        }
    ...
    if (nw.length == 1) {
      for (let k = 0; k < b.length; k++) b[k].classList.remove("bg-gray-700");
      txt =
        "Now we take last remaining element " +
        nw.map((v, i) => {
          if (i != nw.length - 1) return v.innerText;
          else return v.innerText;
        }) +
        " index(" +
        nw.map((v, i) => {
          if (i != nw.length - 1) return v.id[3];
          else return v.id[3];
        }) +
        ").";
      c.innerText = txt;
      ...
      h = nw[0].getBoundingClientRect();
      nw[0].style.transform = `translate(${za.left - h.x}px,${
        ex.bottom - h.bottom - 2.5
      }px)`;
      await new Promise((resolve) => {
        msg.addEventListener("end", () => {
      ...
      bb.replaceChild(placeholderDiv, nw[0]);
      count = 0;
      for (let k = aa.children.length - 1; k >= 0; k--) {
        if (aa.children[k].classList.contains("new")) break;
        count--;
      }
      aa.replaceChild(nw[0], aa.children[aa.children.length + count]);
      nw[0].style.transition = "";
      nw[0].style.transform = "translate(0px,0px)";
      nw[0].style.transition = "all 2s ease-out";
    } else {
      for (k = 0; k < b.length; k++) b[k].classList.remove("bg-gray-700");
      txt =
        "Now we take last remaining elements " +
        nw.map((v, i) => {
          if (i != nw.length - 1) return v.innerText;
          else return v.innerText;
        }) +
      ...
      for (let k = 0; k < nw.length; k++) {
        txt =
          "we take element " + nw[k].innerText + " index(" + nw[k].id[3] + ").";
        c.innerText = txt;
        msg.text = txt;
        speech.speak(msg);
        nw[k].style.backgroundColor = "";
        nw[k].classList.add("bg-red-600");
        const placeholderDiv = document.createElement("div");
        placeholderDiv.classList.add(
          "w-[3rem]",
          "m-[0.2rem]",
        ...
        za = aa.children[aa.children.length + count].getBoundingClientRect();
        h = nw[k].getBoundingClientRect();
        nw[k].style.transform = `translate(${za.left - h.x}px,${
          ex.bottom - h.bottom - 2.5
        }px)`;
        await new Promise((resolve) => {
          msg.addEventListener("end", () => {
        ...
        bb.replaceChild(placeholderDiv, nw[k]);
        count = 0;
        for (let k = aa.children.length - 1; k >= 0; k--) {
          if (aa.children[k].classList.contains("new")) break;
          count--;
        }
        aa.replaceChild(nw[k], aa.children[aa.children.length + count]);
        nw[k].style.transition = "";
        nw[k].style.transform = "translate(0px,0px)";
        nw[k].style.transition = "all 2s ease-out";
      }
    }
    for (k = 0; k < b.length; k++) b[k].classList.remove("bg-gray-700");
    ...
    for (let k = 0; k < bb.children.length; k++) {
      bb.children[k].id = "bar" + k;
    }
    return result.concat(l.slice(leftIndex), r.slice(rightIndex));
  }
