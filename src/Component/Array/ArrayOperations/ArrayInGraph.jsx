import React, { useEffect } from "react";
import { useState, useContext } from "react";
import { arrContext } from "./arr";

export default function ArrInGraph() {
  const arrdetail = useContext(arrContext);
  const [arr, newArr] = useState(arrdetail.array);
  const [arrlen, newArrLen] = useState(arrdetail.arrlen);
  useEffect(() => {
    arr.map((v, i) => {
      document.getElementById('bar'+i).children[0].innerHTML=`<b>${v}</b>`;
    });
  }, [arr]);
  return (
    <>
      <div className="h-[79%] max-h-array flex flex-cols justify-center items-center bg-gray-200 pr-[2rem] mx-[3rem] pb-12 max-h-array">
        <div
          className="w-[50%] h-[10rem] flex justify-center items-end max-sm:hidden max-h-array-ins"
          id="barContainer"
        >
          {arrlen.map((v) => {
            return (
              <div>
                <div
                  className="w-[4rem] text-center border-2 border-black h-[2rem] border-collapse mr-[-1.6px] transition-all duration-[2s] ease-in-out"
                  id={"bar" + v}
                ><div></div></div>
                <div className="relative left-5 text-red-700" id={'title'+v}><b>{v}</b></div>
              </div>
            );
          })}
        </div>
        <dialog
          className="w-[90%] text-[10rem] ml-[4rem] text-center bg-gray-200 sm:hidden vert"
          open
        >
          Please rotate your device to landscape mode for a better user
          experience
        </dialog>
        <dialog
          className="w-[90%] text-[12px] ml-[4rem] text-center bg-gray-200 hidden hori"
          open
        >
          This site can't be run on this device please try another device
        </dialog>
      </div>
      <div className="h-[19%] bg-gray-200 mx-[3rem]" id="extra"></div>
    </>
  );
}
